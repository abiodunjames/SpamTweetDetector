...
class LSTMClassifier(nn.Module):
    ...
    def __init__(self, input_dim, embed_dim, hidden_dim, output_dim,
                 n_layers, bidirectional, dropout, padding_idx, batch_first):
        """
    ...
    def forward(self, tweet, length):
        ...
        packed_embeds = pack_padded_sequence(embeds, length, batch_first=self.batch_first, enforce_sorted=False)

        lstm_out, (hidden, _) = self.lstm(packed_embeds)
        output, output_lengths = pad_packed_sequence(lstm_out, batch_first=self.batch_first)
